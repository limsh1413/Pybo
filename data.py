...
jdata = json.dumps(result)   # 딕셔너리 타입의 데이터를 json 문자열로 바꿔줌
jdata = json.loads(jdata)    # 문자열 형태로 되어있는 json데이터를 json으로 읽을 수 있도록 변형

#1. 결과값으로 받은 제품의 갯수를 파악하세요.
...
# total= []         -이거 내가한거
# for temp in jdata['items']:
#     total.append(temp['title'])
#     total.append(temp['lprice'])
#     total.append(temp['mallName'])
# # print(total)
